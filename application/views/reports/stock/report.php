...
<center><h2 style="text-decoration: underline">Stock :</h2></center><br/>
<center><h3 style="text-decoration: underline">Consumable List</h3></center>
<?php
foreach ($category as $key => $value) {
    $result = $this->db->query("SELECT a.*, c.fld_item_name, (SELECT d.LOOKUP_DATA_NAME FROM sa_lookup_data d WHERE d.LOOKUP_DATA_ID = c.fld_unit ) AS unit, (SELECT b.fld_ledger_page FROM sa_item b WHERE b.fld_id = a.fld_item_id) AS lpage, a.fld_description, (SELECT b.fld_description FROM sa_item b WHERE b.fld_id = a.fld_item_id) AS description FROM im_stock a LEFT JOIN sa_item c ON c.fld_id = a.fld_item_id  WHERE  c.fld_type=215 AND a.fld_category_id = $value->fld_category_id")->result_array();
    if (!empty($result)) {
        ?>
            ...
            <tbody>
                ...
                foreach ($result as $serial => $item) {
                    ...
                    <tr>
                        ...
                        <td style="float: left"><?php echo $item['fld_description']; ?></td>
                        <td style="float: left"><?php echo $item['fld_quantity']; ?></td>
                        <td style="float: left"><?php echo $item['unit']; ?></td>
                        <td style="float: left; margin-left: 10px"><?php echo $item['fld_last_receive_date']; ?></td>
                    </tr>
                    <?php
                }
                ?>
...
<?php
foreach ($category as $key => $value) {
    $result = $this->db->query("SELECT a.*, c.fld_item_name, (SELECT d.LOOKUP_DATA_NAME FROM sa_lookup_data d WHERE d.LOOKUP_DATA_ID = c.fld_unit ) AS unit, (SELECT b.fld_ledger_page FROM sa_item b WHERE b.fld_id = a.fld_item_id) AS lpage, a.fld_description, (SELECT b.fld_description FROM sa_item b WHERE b.fld_id = a.fld_item_id) AS description FROM im_stock a LEFT JOIN sa_item c ON c.fld_id = a.fld_item_id  WHERE  c.fld_type=216 AND a.fld_category_id = $value->fld_category_id")->result_array();
    if (!empty($result)) {
        ?>
        <h3><?php echo "<br/><br/> -------" . "   " . $value->parent . " ---------<br/>"; ?></h3>
        <h3><?php echo " -------" . "   " . $value->category . " " . "---------<br/><br/>"; ?></h3>
